...
ClusterBringupCfg = namedtuple('ClusterBringupCfg',
                               ['n_pods_per_node'])
OspLightCfg = namedtuple('OspLightCfg', ['n_switches', 'n_ports'])
...
def read_config(config):
    ...
    osp_args = config.get('osp_cfg', dict())
    osp_cfg = OspLightCfg(
        n_switches = osp_args.get('n_switches', 4),
        n_ports = osp_args.get('n_ports', 8)
    )
    return global_cfg, cluster_cfg, brex_cfg, osp_cfg
...
def create_logical_switches(ovn, osp_cfg):
    ovn.create_logical_switches(osp_cfg.n_switches)

def create_logical_ports(ovn, osp_cfg):
    for switch in ovn.ls_list:
        print('Creating ' + str(osp_cfg.n_ports) + 'ports for switch: ' + switch.name)
        ovn.create_logical_ports(switch, osp_cfg.n_ports)
...
if __name__ == '__main__':
    ...
    global_cfg, cluster_cfg, brex_cfg, osp_cfg = read_config(config)
    ...
    ovn.create_cluster_router('logical-router')
    create_logical_switches(ovn, osp_cfg)
    create_logical_ports(ovn, osp_cfg)
    ovn.create_cluster_load_balancer('load-balancer')

    for worker in worker_nodes:
        worker.connect(ovn.cluster_cfg)
        worker.ping_ports(ovn, ovn.lp_list)
