...
class Sandbox(object):
    ...
    def _set_container_pid(self):
        stdout = StringIO()
        cmd = 'docker inspect -f {{.State.Pid}} ' + f'{self.container}'
        self.phys_node.run(cmd=cmd, stdout=stdout, raise_on_error=True)
        self.container_pid = stdout.getvalue().strip()


def run(self, cmd="", stdout=None, raise_on_error=False):
    if not self.container_pid and self.container:
        self._set_container_pid()

    if self.container_pid:
        cmd = f'nsenter -t {self.container_pid} -a {cmd}'
    self.phys_node.run(cmd=cmd, stdout=stdout, raise_on_error=raise_on_error)
