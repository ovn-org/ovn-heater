...
class Cluster(object):
    ...
    def create_cluster_join_switch(self, sw_name, cidr, router):
        self.join_switch = self.nbctl.ls_add(sw_name, cidr)

        lrp_ip = netaddr.IPAddress(cidr.last - 1)
        self.join_rp = self.nbctl.lr_port_add(
            router, 'rtr-to-join', RandMac(), lrp_ip,
            cidr.prefixlen
        )
