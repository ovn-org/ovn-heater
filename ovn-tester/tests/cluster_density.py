...
ClusterDensityCfg = namedtuple(
    'ClusterDensityCfg', ['n_runs', 'batch', 'n_startup']
)
    ...
    def run(self, ovn, global_cfg):
        ...
        with Context(ovn, 'cluster_density_startup', brief_report=True) as ctx:
            ...
            ports = ovn.provision_ports(
                DENSITY_N_PODS * self.config.n_startup, passive=True
            )
        ...
        with Context(
            ovn,
            'cluster_density',
            (self.config.n_runs - self.config.n_startup) // self.batch_ratio,
            test=self,
        ) as ctx:
            for i in ctx:
                ns = Namespace(
                    ovn, f'NS_density_{self.config.n_startup + i}', global_cfg
                )
                ns.create_load_balancer()
                ovn.provision_lb(ns.load_balancer)
                all_ns.append(ns)
                ...
                for j in range(self.batch_ratio):
                    ...
                    build_ports = iter_ports[
                        j * DENSITY_N_TOT_PODS : j * DENSITY_N_TOT_PODS
                    ...
                    ports = iter_ports[
                        ...
                        + DENSITY_N_BUILD_PODS : (j + 1) * DENSITY_N_TOT_PODS
                    ]
                    ns.add_ports(ports)
